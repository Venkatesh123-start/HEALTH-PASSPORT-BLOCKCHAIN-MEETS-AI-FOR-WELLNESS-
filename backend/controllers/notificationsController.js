...
/**
 * @desc    Get all notifications for a doctor
 * @route   GET /api/doctor/:doctorId/notifications
 * @access  Private
 */
const getDoctorNotifications = async (req, res, next) => {
  try {
    ...
    const notifications = await Notification.find({ userId: doctorId }).sort({ createdAt: -1 });
...
const getPatientNotifications = async (req, res, next) => {
  try {
    ...
    const notifications = await Notification.find({ userId: patientId }).sort({ createdAt: -1 });
