...
/* =========================================
   HEALTH RISK CALCULATION FOR A PATIENT
========================================= */
router.get("/risk/:patientId", protect, async (req, res, next) => {
  try {
    ...
    if (patient.age) {
      ...
      if (patient.age > 60) {
        riskScore += 20;
        riskFactors.push("Age above 60");

      } else if (patient.age > 45) {
        riskScore += 10;
        riskFactors.push("Age between 45 and 60");
      }
    ...
    if (latestVitals) {
      ...
      /* If vitals missing add small risk */
      riskScore += 5;
      riskFactors.push("No vitals recorded");
    ...
    if (patient.medicalHistory && patient.medicalHistory.length > 0) {
      ...
      const conditions = patient.medicalHistory.map(c => c.toLowerCase());

      highRiskConditions.forEach(cond => {

        if (conditions.some(c => c.includes(cond))) {
          riskScore += 15;
          riskFactors.push(`History of ${cond}`);
        }
    ...
    if (riskScore === 0) {
      riskScore = 10;
      riskFactors.push("Basic health risk baseline");
    }
    ...
    res.json({
      ...
      data: {
        ...
        level,
        factors: riskFactors,
        lastUpdated: latestVitals?.createdAt || new Date(),
        recommendations: getRecommendations(level, riskFactors),
      },
    });
...
function getRecommendations(level, factors) {
  ...
  if (factors.includes("High blood pressure")) {
    rec.push("Monitor blood pressure regularly");
    rec.push("Reduce sodium intake");
  }

  if (factors.includes("High blood sugar")) {
    rec.push("Monitor glucose levels");
    rec.push("Follow a controlled diet");
  }

  if (factors.includes("Elevated heart rate")) {
    rec.push("Manage stress and monitor heart rate");
  }

  if (factors.some(f => f.includes("BMI") || f.includes("Overweight"))) {
    rec.push("Regular exercise recommended");
  }

  if (level === "High") {
    rec.push("Consult a doctor immediately");
  }
  else if (level === "Medium") {
    rec.push("Maintain regular health checkups");
  }
  else {
    rec.push("Maintain healthy lifestyle habits");
  }
