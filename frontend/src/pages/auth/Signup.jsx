...
const Signup = () => {
  ...
  const register = async () => {
    try {
      ...
      if (role === "patient") {
        endpoint = `${process.env.REACT_APP_BACKEND_URL || 'http://localhost:5000'}`/api/patients/register`;
      } else if (role === "doctor") {
        endpoint = `${process.env.REACT_APP_BACKEND_URL || 'http://localhost:5000'}`/api/doctors/register`;
        payload.license = formData.license;
        payload.speciality = formData.speciality;
      } else if (role === "lab") {
        endpoint = `${process.env.REACT_APP_BACKEND_URL || 'http://localhost:5000'}`/api/labs/register`;
        payload.labName = formData.labName;
        payload.licenseNumber = formData.license;
        payload.phone = formData.phone;
        payload.address = formData.address;
      } else if (role === "insurance") {
        endpoint = `${process.env.REACT_APP_BACKEND_URL || 'http://localhost:5000'}`/api/insurance/register`;
